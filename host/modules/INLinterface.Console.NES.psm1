...
function Invoke-NESDump {
	...
	$dumpSuccess = Invoke-NESBasedCartridgeDump -ConsoleFolderName 'nes' -CartridgeName $CartridgeName -PSScriptRoot $HostScriptRoot -LogDir $LogDir -NESmapperMenu $NESmapperMenu -BrowserNesDelayMs 750 -TimesDumped $TimesDumped -LastArgsArray $LastArgsArray -LastCartDest $LastCartDest -LastSramDest $LastSramDest -LastHasSRAM $LastHasSRAM -BaseCartDest $BaseCartDest -BaseSramDest $BaseSramDest -LogFile $LogFile -SessionStartTime $SessionStartTime.Value
