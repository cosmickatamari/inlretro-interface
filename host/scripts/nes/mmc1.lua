...
local function dump_prgrom_with_fallback(file, prg_kb, debug)
	...
	if wrote < need then
		if debug then print(string.format("[MMC1] original PRG wrote %d (need %d) -> retrying in mode 2", wrote, need)) end
...
--test cart by reading manf/prod ID
	if test then
		...
		-- (disabled) nes.ppu_ram_sense(0x1000, true)
...
--dump the cart to dumpfile
	if read then
		...
		dump_prgrom_with_fallback(file, prg_size, false)
