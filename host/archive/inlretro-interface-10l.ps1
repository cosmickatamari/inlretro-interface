...
function Show-FileAnalysis {
	...
	foreach ($line in $analysisText) {
		...
		} elseif ($line -match "^Leading padding|^First non-zero|^Note:") {
...
function Show-Header {
	...
	$asciiLines = @(
		...
		"                                                        Last Updated: 11/2/2025 | ver. 10L",
		"------------------------------------------------------------------------------------------"
	)
...
function ReDump {
	...
	while ($true) {
		...
		if ($rerun) {
			...
			$null = Invoke-INLRetro -ArgsArray $newArgsArray -CartDest $newCartDest -SramDest $newSramDest -HasSRAM $script:LastHasSRAM -IsRedump $true -CartridgeTitle ""
