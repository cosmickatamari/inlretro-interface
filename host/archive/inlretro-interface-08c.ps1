...
if ($PSVersionTable.PSVersion.Major -lt 7) {
    Write-Host "This script requires at least PowerShell 7." -ForegroundColor Red
    exit 1
}
...
# Directories are created on-demand when dumping cartridges
...
function Show-Header {
    ...
    Write-Host "`nCreated By:    Cosmic Katamari"
    Write-Host "Twitter/X:     @cosmickatamari"
    Write-Host "Last Released: 10/12/2025"
    Write-Host "Version:       0.8c"
    Write-Host "-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-"
...
function ReDump {
	# Check if we have parameters from a previous dump
	...
	while ($true) {
		...
		if($rerun) {
			...
			Invoke-INLRetro -ArgsArray $newArgsArray -CartDest $newCartDest -SramDest $newSramDest -HasSRAM $script:LastHasSRAM
