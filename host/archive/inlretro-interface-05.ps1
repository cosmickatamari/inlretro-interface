...
function Select-Mapper {
	if (-not $script:NESCARTDBOpened) {
		Start-Process "https://nescartdb.com/search" -WindowStyle Minimized
		$script:NESCARTDBOpened = $true
	}

    Write-Host "`nYour default web browser has opened the NES database. `nUsing the (Game Title) search field, retrieve detailed information about your game cartridge." -ForegroundColor Blue
