...
# Import modules in dependency order (-Force: reload on every launch so edits to .psm1 are picked up
# in the same PowerShell session; otherwise Import-Module keeps the previously loaded copy).
...
while ($true) {
	...
	try {
		...
		if ($sys -eq 'Nintendo 64') {
			Write-Host "Save data (EEPROM / SRAM / Flash) is not supported on this dumper (hardware limitation)." -ForegroundColor DarkRed
			Write-Host "Output is Big-Endian formatted ROM only (.z64).`n" -ForegroundColor DarkYellow
		}
