...
Import-Module "$PSScriptRoot\modules\INLinterface.Utility.psm1" -DisableNameChecking
Import-Module "$PSScriptRoot\modules\INLinterface.Window.psm1" -DisableNameChecking
Import-Module "$PSScriptRoot\modules\INLinterface.Browser.psm1" -DisableNameChecking
Import-Module "$PSScriptRoot\modules\INLinterface.FileAnalysis.psm1" -DisableNameChecking
Import-Module "$PSScriptRoot\modules\INLinterface.Logging.psm1" -DisableNameChecking
Import-Module "$PSScriptRoot\modules\INLinterface.UI.psm1" -DisableNameChecking
Import-Module "$PSScriptRoot\modules\INLinterface.Dumping.psm1" -DisableNameChecking
Import-Module "$PSScriptRoot\modules\INLinterface.Consoles.psm1" -DisableNameChecking
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
	...
	Write-Host "`nThis script requires PowerShell 7 or higher." -ForegroundColor Yellow
	Write-Host "Your current version: $($PSVersionTable.PSVersion)" -ForegroundColor Magenta
	
	if (Read-YesNo "`nWould you like to download and install the latest PowerShell version?") {
...
try {
	...
	$errorMessage = @"
Failed to load required data files from $dataDir

Please ensure the following files exist:
  - nes-mappers.json

The latest release can be downloaded at: https://github.com/cosmickatamari/inlretro-interface
